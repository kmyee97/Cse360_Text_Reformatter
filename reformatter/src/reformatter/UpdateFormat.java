...
public class UpdateFormat {
	...
	public static int[] updateFormat(String filename, int justification, int length, String outputName) {
        ...
        int lineCount = 0;
		 try {
	            ...
	            WriteLine(toWrite, justification, length, outputName);
	        }
	...
	static void WriteLine(String[] writeArray, int justification, int length, String outputName) {
		 try {
			...
			if(justification == 1) { //if right justified
        		...
        		System.out.println("Full justified being used");
        		for(int i = 0; i < writeArray.length; i++) {
        			String tempString = writeArray[i];
    				while(tempString.length() < length) {
    					for(int j = 0; j < tempString.length(); j++) {
    						System.out.println("Top of loop j = " + j);
